...
def cropConvert(samples_pair, crop_dimension, original_dimension, writer):
    for image, depth in samples_pair:
        for i in range(18, original_dimension[1]-crop_dimension[1], crop_dimension[1]):
            for j in range(18, original_dimension[0]-crop_dimension[0], crop_dimension[0]):
                image_np = np.array(image[:, i:i + crop_dimension[1], j:j + crop_dimension[0]])
                depth_np = np.array(depth[i:i + crop_dimension[1], j:j + crop_dimension[0]])
                image_np = np.swapaxes(image_np,0,2) # C x W x H => H x W x C
                depth_np = np.swapaxes(depth_np,0,1) # W x H => H x W
...
train_length = int(0.9 * images.shape[0])
